...
while key != ESC:
    win.addstr(0, 26, 'Score ' + str(score) + ' ')
    ...
    if key not in [curses.KEY_LEFT, curses.KEY_RIGHT, curses.KEY_UP, curses.KEY_DOWN, ESC,WKEY,AKEY,SKEY,DKEY ]:
        key = prev_key

    # calculate the next coordinates
    y = snake[0][0]
    x = snake[0][1]
    if key == curses.KEY_DOWN or key == SKEY:
        y += 1
    if key == curses.KEY_UP or key == WKEY:
        y -= 1
    if key == curses.KEY_LEFT or key == AKEY:
        x -= 1
    if key == curses.KEY_RIGHT or key == DKEY:
        x += 1
